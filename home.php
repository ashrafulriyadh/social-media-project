...
<div class="container-fluid">
		<div class="row">
    		...
    		";
    			?>
					<div class="col" id="sticky"> <div class="card" style="width: 16rem;background-color:#ADD9E6;">
					  <div class="card-body">
					    <h5 class="card-title"> Hey, <?php $user = $_SESSION['user_email'];
					                $get_user = "select * from users where user_email='$user'";
					                $run_user = mysqli_query($con,$get_user);
					                $row = mysqli_fetch_array($run_user);
					                ...
					                $user_name = $row['user_name'];
					echo "$user_name"; ?> </h5>
					    <p class="card-text">Welcome to your profile here you can find everything you need</p>
					  </div>
					    </div>
					</div>
				</div>
        ...
        <div class="col-sm-7 col-lg-10 mt-5">
    		<div id="timeline">
    			<form id="form" action="home.php?id=<?php echo $user_id;?>" method = "post" id="f">
    				<h2> Post on your timeline </h2>
    				<input class="form-control" type="text" name="title" placeholder="Write your title...." size="81" required="required" id="in"><br/>
    				<textarea class="form-control" cols="83" rows="4" name="content" placeholder="Write details...."></textarea><br/>
    				<select class="form-control" style="color:black" name="topic">
    					<option>Select Topic</option>
    					<?php getTopics();?>
    				</select>
    				<input class="btn btn-primary" type="submit" name="sub" value="Post to Timeline" id="btn">
    			</form>
    			<?php insertPost(); ?>

    			<div id="post"><h3 id="title">Most Recent Discussions! </h3>
    			<?php get_posts();?>
            </div>
